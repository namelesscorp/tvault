...
- **Unlock with Touch ID (macOS).** When creating a vault you can now save its key to the macOS Keychain and reopen the vault later with Touch ID or your Mac login password, instead of entering credentials by hand. It works for every unlock method — password, master token and Shamir shares — and, when the vault uses HMAC integrity, its integrity password is saved alongside the key, so a single Touch ID prompt opens the vault completely. The option is opt-in, offered on its own step in the create flow, and the saved key is removed from the Keychain when you delete the vault. macOS only; the feature is hidden on other platforms.
...
[1.1.0]: https://github.com/namelesscorp/tvault/releases/tag/v1.1.0
