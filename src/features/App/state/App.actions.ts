...
import { createAsyncOnceGuard, devError, devInfo } from "utils";
...
const appInitImpl = async (dispatch: AppDispatch, getState: AppGetState) => {
	...
	try {
		...
		try {
			...
			const checkPromises = recentList.map(async item => {
				try {
			...
			const pathsToRemove = results.filter(Boolean) as string[];
